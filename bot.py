...
BOT_VERSION = "0.9.6" # Версия по твоему запросу
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"👋 Крылоха {BOT_VERSION} запущен! Напиши /help.")

async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        f"📜 **КОМАНДЫ ({BOT_VERSION})**\n\n"
        "🕹 Меню: /start, /help, /magaz\n"
        "💰 Экономика: баланс (б), бонус, тп, обо мне\n"
        "💸 Передача: передать [сумма] (ответом)\n"
        "🛡 Модер: инфа, молчи [время], скажи, бан [время], варн (ответом)\n"
        "🎲 Игра: Рулетка (ответом)\n\n"
        "💡 *Причину пиши с новой строки после команды.*"
    )
    await update.message.reply_text(text, parse_mode="Markdown")
...
async def shop_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "buy_unmute":
        if u_id == OWNER_ID or bal >= 1000:
            ...
            try:
                # Сброс статуса для магазина тоже
        ...
        if u_id == OWNER_ID or bal >= 500:
            ...
            else: await q.answer("❌ Нет варнов.")
        else: await q.answer("❌ Мало KLC!", show_alert=True)
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if first_line in ["баланс", "б"]:
        ...
        await update.message.reply_text(f"💰 Баланс: {bal} KLC")

    elif first_line == "бонус":
        now = datetime.now()
        if user.id in bonus_timers and now < bonus_timers[user.id] + timedelta(days=1):
            return await update.message.reply_text("❌ Приходи через 24 часа!")
        ...
        save_json(ECONOMY_FILE, user_balance)
        await update.message.reply_text(f"🎁 Бонус: +{amt} KLC!")

    # МОДЕРАЦИЯ
    if main_cmd in ["молчи", "скажи", "бан", "варн", "инфа", "передать"]:
        if not msg: return await update.message.reply_text("⚠️ Ответь на сообщение пользователя этой командой!")
        ...
        if main_cmd == "передать":
            try:
                ...
                if user.id != OWNER_ID and user_balance.get(user.id, 0) < amount: return await update.message.reply_text("❌ Недостаточно средств")
        ...
        if c_rank >= 1 and (t_rank < c_rank or user.id == OWNER_ID):
            try:
                # Время
                delta = timedelta(hours=1); t_txt = "1 час"
                if len(cmd_parts) > 1:
                    try:
                        val = int(cmd_parts[1])
                        unit = cmd_parts[2].lower() if len(cmd_parts) > 2 else "ч"
                        if unit.startswith("д"): delta = timedelta(days=val); t_txt = f"{val} дн."
                        elif unit.startswith("м"): delta = timedelta(minutes=val); t_txt = f"{val} мин."
                        else: delta = timedelta(hours=val); t_txt = f"{val} час."
                    except: pass
                ...
                if main_cmd == "молчи":
                    ...
                    await update.message.reply_text(f"🤫 Тишина на {t_txt}.\n📝 Причина: {reason}")
                
                elif main_cmd == "скажи":
                    # ГАРАНТИРОВАННЫЙ РАЗМУТ
                    await context.bot.unban_chat_member(chat_id, t_id, only_if_banned=False)
                    await context.bot.restrict_chat_member(chat_id, t_id, permissions=ChatPermissions(can_send_messages=True, can_send_media_messages=True, can_send_other_messages=True, can_send_polls=True))
                    ...
                    await update.message.reply_text(f"🚫 Бан на {t_txt}.\n📝 Причина: {reason}")
                ...
                await update.message.reply_text("❌ Ошибка прав. Проверь админку бота.")
...
async def roulette_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🎲 **ДУЭЛЬ!**\n{p1.first_name} VS {p2.first_name}\nВыбирай ставку:", reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")
...
async def rt_action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if any(l <= 0 for l in g['lives'].values()):
        ...
        try:
            if g['bet_type'] == "mute": await context.bot.restrict_chat_member(q.message.chat_id, loser_id, permissions=ChatPermissions(can_send_messages=False), until_date=datetime.now()+timedelta(hours=1))
            elif g['bet_type'] == "ban": await context.bot.ban_chat_member(q.message.chat_id, loser_id, until_date=datetime.now()+timedelta(days=1))
        except: pass
        await q.edit_message_text(f"{msg}\n\n🏆 **Победил {w_name}!**", parse_mode="Markdown")
