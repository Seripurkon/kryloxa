...
OWNER_ID = 5679520675
VERSION = "2.1 (Glaz Character HardClick)"
...
def extract_player_count_from_ws(messages):
    ...
    for raw in messages:
        ...
        # Иногда JSON лежит внутри socket.io формата: 42["event",{...}]
...
async def choose_character(page):
    """
    Жёсткий выбор персонажа для Gartic Phone.
    Иногда персонажи рисуются не обычными DOM-элементами, поэтому кликаем
    и по найденным элементам, и по координатам в зоне выбора персонажа.
    """
    try:
        logging.info("[GLAZ] CHARACTER SELECT START")

        # 1) Сначала пробуем нормальные DOM-элементы
        selectors = [
            "[class*='character']",
            "[class*='Character']",
            "[class*='avatar']",
            ...
            try:
                ...
                for i in range(min(total, 80)):
                    try:
                        ...
                        # Не кликаем по огромному фону/странице
                        if 25 <= w <= 300 and 25 <= h <= 300 and y < 650:
                            await el.click(force=True)
                            logging.info(f"[GLAZ] CHARACTER CLICKED DOM: {selector} #{i} box={box}")
        ...
        # 2) Если DOM не помог — кликаем по типичным местам зоны персонажа
        # viewport 1280x720; персонаж обычно над полем ника/кнопкой входа
        points = [
        ...
        logging.info("[GLAZ] CHARACTER SELECT END")
        return True
...
async def click_join_button(page):
    """
    Более надёжное нажатие ВОЙТИ/JOIN.
    """
    try:
    ...
    try:
        # JS-клик по элементам с текстом
...
async def check_gartic_link(url, count_mode=False):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            def on_ws(ws):
                ...
                def save_sent(payload):
                    try:
                        ...
                        if payload:
                            ws_messages.append(payload[:20000])
                            logging.info(f"[WS SENT] {payload[:600]}")
                    except Exception as e:
                        logging.info(f"[WS SENT ERROR] {e}")

                ws.on("framereceived", save_received)
                ws.on("framesent", save_sent)
            ...
            room_code_match = re.search(r"garticphone\.com/(ru/)?[a-z0-9]{6,}", final_url)

            if not count_mode:
                if room_code_match:
                    return "⚠️ Глаз: Ссылка похожа на комнату.\nДля точной проверки используй `статус+`."
                return "⚠️ Глаз: Не смог точно определить статус."
            ...
            try:
                if not await input_box.is_visible(timeout=9000):
                    ...
                    if await check_dead_room(page):
                        return "❌ Глаз: Ссылка мертва (Комната закрыта)."
                    return "⚠️ Глаз: Не нашёл поле входа. Смотри логи [GLAZ NO INPUT]."
            except Exception:
                await debug_page_state(page, "[GLAZ INPUT ERROR]")
                if await check_dead_room(page):
                    return "❌ Глаз: Ссылка мертва (Комната закрыта)."
                return "⚠️ Глаз: Не нашёл поле входа. Смотри логи [GLAZ INPUT ERROR]."
            ...
            # Дублируем ввод через JS + события, чтобы React/Gartic точно увидел ник
            try:
                await page.evaluate(
                    """(nick) => {
            ...
            return "⚠️ Глаз: Не смог подтвердить вход. Смотри логи [GLAZ AFTER WAIT]."
...
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in ["баланс", "б"]:
        balance = "∞ (Owner)" if user.id == OWNER_ID else f"{user_balance.get(user.id, 0)} KLC"
    ...
    if text == "обо мне" or (text == "инфа" and reply):
        ...
        balance = "∞ (Owner)" if target.id == OWNER_ID else f"{user_balance.get(target.id, 0)} KLC"

        await update.message.reply_text(
            f"👤 Профиль пользователя {target.first_name}:\n"
            f"🆔 ID: `{target.id}`\n"
            f"⭐️ Ранг: {get_rank_str(target.id)}\n"
            f"💰 Баланс: {balance}\n"
    ...
    if text.startswith("промо "):
        ...
        if code in promos_data:
            ...
            save_json("economy", user_balance)
            save_json("promos", promos_data)

            await update.message.reply_text(f"✅ Промокод `{code}` активирован! +{promo['amt']} KLC.", parse_mode="Markdown")
        else:
    ...
    if reply and user_ranks.get(user.id, 0) >= 1:
        ...
        if text.startswith("варн"):
            ...
            if warns[target_id] >= 3:
                ...
                await update.message.reply_text(
                    f"⚠️ Варн {target_name} ({warns[target_id]}/3).\n"
                    f"Причина: {reason}"
                )
...
async def on_call(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("shop_"):
        ...
        if data == "shop_unmute":
            if user_balance.get(uid, 0) >= 1000:
