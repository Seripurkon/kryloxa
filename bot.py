    ...
    if q.data == "buy_unmute":
        ...
        if u_id == OWNER_ID or bal >= 500:
            if warns.get(u_id, 0) > 0:
                ...
                await q.answer("❌ У вас нет варнов.")
        else:
            await q.answer("❌ Недостаточно KLC!", show_alert=True)
    ...
    if msg:
        ...
        if main_cmd == "передать":
            try:
                ...
                await update.message.reply_text("❌ Формат: передать 100")
            return
...
async def roulette_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    roulette_games[g_id] = {"p1": p1.id, "p1_n": p1.first_name, "p2": p2.id, "p2_n": p2.first_name,
                            "lives": {p1.id: 2, p2.id: 2}, "turn": p2.id, "bet_type": None}
    text = (
        f"🎲 **ДУЭЛЬ!**\n\n👤 {p1.first_name} VS {p2.first_name}\n🔫 Барабан: 6 патронов\n\n👉 {p2.first_name}, выбирай ставку:")
    kb = [[InlineKeyboardButton("Варн", callback_data=f"rbet_warn_{g_id}"),
           InlineKeyboardButton("Мут", callback_data=f"rbet_mute_{g_id}")],
...
async def update_ui(q, g_id, status):
    ...
    txt = (
        f"🎰 Ставка: {g['bet_type'].upper()}\n🔫 Патроны: {len(g['chamber'])} (🔥 {l} | ❄️ {bl})\n\n📢 {status}\n👤 {g['p1_n']}: {'❤️' * g['lives'][g['p1']]}\n👤 {g['p2_n']}: {'❤️' * g['lives'][g['p2']]}\n👉 Ход: {t_n}")
    kb = [[InlineKeyboardButton("🎯 Оппонент", callback_data=f"rt_opp_{g_id}"),
           InlineKeyboardButton("🔫 Себя", callback_data=f"rt_self_{g_id}")]]
    ...
    if any(l <= 0 for l in g['lives'].values()):
        ...
        try:
            if bet == "warn":
                ...
                await context.bot.restrict_chat_member(q.message.chat_id, loser_id,
                                                       permissions=ChatPermissions(can_send_messages=False),
                                                       until_date=datetime.now() + timedelta(hours=1))
                pun = f"🤫 {l_name} в МУТЕ на час!"
            elif bet == "ban":
                await context.bot.ban_chat_member(q.message.chat_id, loser_id,
